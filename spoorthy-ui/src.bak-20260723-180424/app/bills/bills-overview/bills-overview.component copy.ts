...
export class BillsOverviewComponent implements OnInit {
  ...
  attendenceMonthColumn = ['OTEmp', 'OTBasedOn', 'EmployeeID', 'EmployeeName', 'Designation', 'Salary', 'NoOfDaysWorked', 'NoOfLeaves', 'OTDays',
  ...
  SearchFilter(str: string) {
    var filterArr = this.employeeData.filter((obj: any) => {
      return obj?.FullName.toLowerCase().includes(str.toLowerCase()) || obj?.EmployeeId.toString().includes(str)
    });
